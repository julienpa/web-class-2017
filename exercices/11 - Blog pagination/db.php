...
function get_nb_articles() {
  global $mysqli;
  $query = $mysqli->query('SELECT count(id) as nb_articles FROM article');
  $obj = $query->fetch_object();
  return $obj->nb_articles;
}

function get_articles($nb_articles, $page) {
  global $mysqli;

  $start = $nb_articles * ($page - 1);

  $query = $mysqli->query("SELECT * FROM article LIMIT $start, $nb_articles");
  $articles = [];
...
function get_article_by_id($id) {
  global $mysqli;
  $query = $mysqli->query("SELECT * FROM article WHERE id = $id LIMIT 1");
